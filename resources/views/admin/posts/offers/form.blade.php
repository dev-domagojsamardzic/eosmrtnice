        ...
        <form method="POST" action="{{ $action }}">
            ...
            <div class="row">
                <div class="col-lg-8 col-sm-12 d-flex align-items-start justify-content-end flex-column">
                    <span class="font-weight-bold">{{ __('models/post.type') }}: {{ $post->type->translate() }}</span>
                    <span>{{ $post->deceased_full_name_lg }}</span>
                    <span>{{ __('models/post.size') }}: {{ ucfirst($post->size->name) }} ({{ __('common.to') . ' ' . $post->size->value . ' ' . __('common.words') }})</span>
                    <span>{{ __('models/post.is_framed') }}: {{ $post->is_framed ? __('common.yes') : __('common.no') }}</span>
                    <span>{{ __('models/post.starts_at') }}: {{ $post->starts_at->format('d.m.Y.') }}</span>
                </div>
